...
function addSkillRecursive(style_id, skill) {
  addStatement(
    `INSERT INTO skills
     VALUES (${style_id}, ${skill.id}, "${skill.label}", "${skill.effect}", "${skill.target_type}", ${skill.hit_count},
  ...
  for (const part of skill.parts) {
    addStatement(
      `INSERT INTO parts
       VALUES (${skill.id}, "${part.skill_type}", ${part.diff_for_max}, ${part.multipliers.dp}, ${part.multipliers.hp},
               ${part.multipliers.dr},
               ${part.parameters.str}, ${part.parameters.dex}, ${part.parameters.con}, ${part.parameters.spr},
               ${part.parameters.wis}, ${part.parameters.luk},
               ${part.power[0]}, ${part.power[1]}, ${part.growth[0]}, ${part.growth[1]}, ${part.value[0]},
               ${part.value[1]},
...
function addPassive(style_id, limit_break_step, passive) {
  addStatement(
    `INSERT INTO passives
     VALUES (${style_id}, ${limit_break_step}, "${passive.label}", "${passive.effect}", "${passive.timing}",
             "${passive.condition}");`,
  )
}
