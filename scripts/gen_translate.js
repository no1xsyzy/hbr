...
function makeTranslate(styles, bili_roles) {
  const translation = {}
...
const stylesFn = './data_hbr/styles.json'
const biliRolesFn = './data_bili/role_details.json'
const outfile = './data_gen/translation.json'

if (check_gen({ froms: [stylesFn], tos: [biliRolesFn] })) {
  const styles = JSON.parse(readFileSync(stylesFn, 'utf-8'))
  const bili_roles = JSON.parse(readFileSync(biliRolesFn, 'utf-8'))
  const translation = makeTranslate(styles, bili_roles)
  mkdirSync('./data_gen', { recursive: true })
  writeFileSync(outfile, JSON.stringify(translation, null, 2))
}
